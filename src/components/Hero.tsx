...
const slides = [
  {
    ...
    image: 'https://images.unsplash.com/photo-1550614000-4b95d4eb6744?auto=format&fit=crop&q=80',
    isVideo: true
  },
  {
    ...
    <div className="relative h-screen min-h-[600px] w-full overflow-hidden">
        ...
        >
          ...
          <div className="absolute inset-0 flex items-end pb-24 md:pb-32 px-6 md:px-16 max-w-7xl mx-auto w-full">
            <div className="max-w-2xl text-left animate-in slide-in-from-bottom-8 duration-700 fade-in fill-mode-both">
              <h2 className="text-4xl md:text-5xl lg:text-[56px] font-light mb-4 text-white leading-tight">
                {slide.title}
              </h2>
              <p className="text-base md:text-lg mb-8 text-white/90 font-light max-w-lg">
                {slide.subtitle}
              </p>
              <button className="px-8 py-3.5 border border-white text-white text-sm font-medium tracking-widest hover:bg-white hover:text-black transition-colors rounded-full uppercase">
                {slide.cta}
              </button>
            </div>
          </div>
          ...
          >
            <div
              className={`h-0.5 transition-all duration-300 ${
                index === currentSlide ? 'w-8 bg-white' : 'w-4 bg-white/50 group-hover:bg-white/80'
              }`}
            />
          </button>
        ))}
      ...
      {slides[currentSlide].isVideo && (
        <button
          ...
          className="absolute bottom-8 right-32 md:right-48 z-20 text-white/70 hover:text-white p-2 transition-colors"
